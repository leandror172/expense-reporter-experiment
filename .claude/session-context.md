...
- **PR #32 OPEN** (`feat/type-routing-improvements` → master): classifier emits type (5.R4-classifier, 41afa56) + RUI-4 type CSV column + review serialization fix (`sheets`→`types`) + full-cycle acceptance. 524 unit tests + acceptance green; validated in-browser.
- **5.R4 historical extraction — DONE (this session):** one-off Python extraction (`.claude/scratch/{extract_old_workbooks,dedup_corpus,build_corpus,build_logs}.py`) of the 2022–2025 old workbooks → 1808 deduped records. **Training corpus augmented 694→1788** (`training_data_complete.json`, gitignored, backed up). **Per-year expense logs** built (`expenses_log-{2022,2023,2024}.jsonl` + 2025 merged into base), all route through `generate-workbook`. Taxonomy ~98.8% aligned; 4-tuple alias map; taxonomy.json left canonical. Details: memory `project_workbook_extraction_5r4`.
- **PR #33 OPEN** (`chore/extraction-gitignore-memory` → master, stacked on #32): `.gitignore` widened for per-year logs + `*.bak-*` personal-data backups; stale type-emission memory corrected.
- **Data-leak (session 33):** remediated — runtime logs gitignored, history rewritten/force-pushed. Local backup at `~/expense-data-backup-20260619-121709/`.
- **Next:** merge PR #32 then #33; then year-rollover (T-03), year adaptation (T-11), retire bare-name fallback (T-09, now gated on income routing), 5.R4 id collision (T-10), TF-IDF (5.R1).
...
|------|-----------|-------|
| **Merge PR #32 then #33 (START HERE)** | `gh pr view 32`; `gh pr view 33` | #32 = type-routing (classifier type + RUI-4 + review fix + full-cycle acceptance), green & in-browser-validated. #33 = gitignore + memory sync, stacked on #32 — review after #32 merges. Merge both, then continue below. |
...
| Year adaptation (T-11) | `internal/taxonomy/loader.go` `parseDate` (requires exactly `DD/MM`); memory `project_workbook_extraction_5r4` | Make expenses_log carry year: accept `DD/MM/YYYY` + generate use per-entry year (fallback `--year`). Collapses the per-year log split from 5.R4 back into one multi-year log. |
| Retire bare-name fallback (T-09) | `[ref:taxonomy-identity-key]`; `internal/taxonomy/loader.go` `scanEntries`; stderr fallback count; taxonomy `.memories/KNOWLEDGE.md` | 5.R4 made the type-less EXPENSE surface ~0. Retirement now gated on: income is structurally type-less and shares `byName` → needs its own `incomePath` route first; `add`/`correct` still emit type-less (to classifications.jsonl only). Not gated on the classifier anymore. |
| Fix 5.R4 id collision (T-10) | `.claude/tasks.md` (`5.R4` = historical extraction, now DONE) vs the classifier-emits-type plan/status | Two things shared id `5.R4`; the extraction one is now done. Rename so the board and plans agree. |
| Generate full-suite flake (T-08, low) | `test/.memories/QUICK.md` (Infrastructure timeout gotcha); `test/verify/workbook_structure.go` | `TestGenerateWorkbook_Skeleton` fails only under full-suite timeout, passes isolated. Benign; investigate only if it recurs isolated. |
| 5.R1 (TF-IDF layer) | `project_r1_evaluation_procedure.md` memory; `data/classification/research_insights.md` | Instrumentation prerequisite still open. Note: corpus now 1788 (richer TF-IDF vocabulary). |
<!-- /ref:session-reading-guide -->
