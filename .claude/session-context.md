...
- **T-13 — DONE (session 41), PR #36 open → master, REVIEWED session 42.** The classifier predicts the full `(type,category,subcategory)` path against `taxonomy.json` via an atomic 112-path enum; `Result.Type`; `classifier.Taxonomy`/`LoadTaxonomy` deleted (feature dict now keyword-only). New `internal/taxonomy/path.go` helpers (`PathEnum`/`Split`/`PathFor`/`ResolveLeaf`/`TypesForLeaf`/`CategoryForLeaf`). `add` resolves via taxonomy + `--type`/prompt/non-interactive-error hybrid; `auto`/`batch-auto` write the predicted type. Report: `.claude/t13-implementation-report.md`.
- **PR #36 review follow-ups — session 42 (3 commits: `f997ea7`, `3dca7b9`, `9962e5c` + docs `8c18a79`):** (1) **default model reverted qcoder→`my-classifier-q3`** for ALL commands — enum validity is GRAMMAR-enforced (Ollama `format`→GBNF, model-independent), so qcoder (20.7 GB, doesn't fit 12 GB GPU) added only latency + load 500s; q3 validated end-to-end on the real 112-enum. q3 accurate but slow (~12 s/call → T-14). (2) **T-15 Slice 1**: predicted `type` surfaced in `CandidateOutput`/`AddOutput` JSON (was dropped at the MCP boundary; Slice 2 `--predicted-type` deferred). (3) **Acceptance repaired (T-17×11 + T-18×2)** — T-13 made `taxonomy_path` mandatory but many `Given`s never set it (build-tag hid it from `go test ./...`); plus a pre-existing `correct` seed-id bug (T-11 date norm). All 13 green via grouped runs; full suite needs `-timeout 30m` (q3 slow). Postmortem: `.claude/session42-postmortem.md`.
- **WS-B (commands → log-append) — IN PROGRESS:** `add` (slice 1) + `auto` (slice 2) DONE and retrofitted to predicted-path resolution by T-13. New `internal/appender.ExpandAndAppend` = single append-time writer; installments expand at append time; no workbook writes. **Remaining: slice 3 `batch-auto` (Classify already emits type — drop the insert branch), slice 4 `apply` (delete write-half).**
- **Next:** WS-B slice 3 `batch-auto` → log-append, slice 4 `apply` → log-append; then WS-D (retire bare-name fallback), WS-E (delete dead insert code). PR #36 follow-ups: **T-14** (model accuracy+speed benchmark q3/q35/qcoder), **T-16** (CLAUDE.md qcoder-tier doc cleanup), **T-19** (enum "none-of-these" escape hatch). Open: promote merged `expenses_log-allyears.jsonl` to canonical + retire per-year split (deferred).
...
|------|-----------|-------|
| **WS-B slice 3 `batch-auto` → log-append (START HERE)** | retire-insertion plan "WS-B progress"; `cmd/.../batch_auto.go`; `internal/appender/appender.go`; T-13 report `.claude/t13-implementation-report.md` | T-13 already wired `batch-auto`'s `Classify` to the predicted full path + `classifiedRow.Type` from `top.Type`. Now drop the workbook-insert branch (`insertClassified`/`workflow.InsertBatchExpensesFromClassified`) + `rollover.csv`; keep classified/review CSV → review → apply; reuse `ExpandAndAppend`. **Run target tests in GROUPS with `-v` — q3 is slow (~12 s/call), the full suite exceeds the 600 s default; use `-timeout 30m` or sub-suites (session 42).** |
| **PR #36 review follow-ups (T-14 / T-16 / T-19)** | `.claude/session42-postmortem.md`; classifier `.memories/KNOWLEDGE.md` (grammar-enforcement) | T-14: benchmark accuracy+speed across q3/q35/qcoder on real labeled data, set the smallest-that-fits default. T-16: scrub CLAUDE.md's "qcoder primary/required for 5.7+" claim. T-19: design a sentinel/`Diversos` escape so the enum can decline out-of-domain inputs (currently force-mapped at high confidence). |
