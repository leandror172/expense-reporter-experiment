...
- **T-22 — DONE (session 47), PR #42 OPEN** (`feat/t22-type-descriptions`). Type-level taxonomy descriptions merged into the classifier prompt from a tracked sidecar `config/type-descriptions.json`. No-think A/B: **English +6.0 pp TYPE accuracy** (74.3→80.3), +2.6 pp full-path; Portuguese dropped; **calibration UNCHANGED — not a gate fix**. Think-on confirmation deferred (T-26); leaf-level extension = T-27. Report addendum "T-22 Addendum".
- **Next:** **T-23** calibration/gate rethink (the WS-D blocker — T-22 confirmed descriptions don't fix the gate) → **WS-D (T-09)** retire bare-name fallback → **WS-E** delete dead insert code. Open: merge PR #42; T-26 (T-22 think-on run) · T-27 (leaf-level descriptions) · T-28 (alt classification strategies) · T-20 dedup · T-21 installment under-recording · T-24 --think default · T-25 q35 grammar-bug upstream + persona cleanup; promote merged all-years log (deferred).
...
|------|-----------|-------|
| **Merge PR #42 (T-22)** | PR #42; branch `feat/t22-type-descriptions` | Type-descriptions via tracked sidecar. Point at master, don't fuss over base ([[feedback_pr_base_no_rebase_fuss]]). |
| **T-23 — calibration/gate rethink (blocks WS-D, do FIRST)** | `.claude/t14-benchmark-report.md` (calibration section + T-22 addendum); `internal/classifier/decision.go`; `config.json` `auto_insert_excluded` | Confidence is uninformative (91% of errors ≥0.85). **T-22 confirmed descriptions do NOT fix this.** Options: threshold on margin/agreement instead of raw confidence, few-shot pairs with varied confidence (0.95 anchor suspicion), review-first default for non-recurring items (recurrence is a better safety signal than confidence), stronger exclusions. |
| **T-26 — T-22 think-on confirmation run** | `.claude/t14-benchmark-report.md` "T-22 Addendum"; `run_t22.sh` header (⚠ stale-mechanism note); `project_t22_type_descriptions` | Confirm the +6 pp type gain holds think-on AND measure OOD sentinel decline WITH descriptions (only alive in think-on). **Must toggle the SIDECAR** (`config/type-descriptions.json` present=descen, absent=nodesc), NOT swap `taxonomy.json` — rework `run_t22.sh` first. ~72 min. Couples T-23/T-24. |
| **T-27 — leaf-level descriptions (extends T-22)** | T-22 addendum; `writeTaxonomyTree`/`descriptions.go`; `config/type-descriptions.json` | Type-level descriptions left "wrong leaf" errors untouched. Per-leaf descriptions (×112) target those, but need prompt-size/latency measurement before adopting. Authoring-home + sidecar schema would extend beyond the flat type→desc map. |
| **T-28 — alternative classification strategies** | `internal/classifier/classifier.go` (prompt + `format`/GBNF); T-13 report; t14 addendum | Explore beyond one-shot atomic-enum: (a) two-stage — classify type (and/or category) first, then derive the subcategory within it; (b) two-pass grammar — classify UNCONSTRAINED first, then re-ask WITH the grammar only if the free-form answer doesn't match the tree. Measure vs the current baseline on the T-14 sample. |
| **WS-D — retire bare-name fallback (T-09, after T-23)** | `[ref:taxonomy-identity-key]`; `internal/taxonomy/loader.go` `scanEntries`; stderr fallback count | **Gate NOT passed in T-14** — 37% wrong full paths behind a gate that stops ~9% of them. Requires T-23 + real-data type-less surface ~0. |
...
| --think default decision (T-24, after T-26) | `.claude/t14-benchmark-report.md`; T-22 addendum | 10× speed vs −3.3 pp + dead sentinel. If T-23 moves safety off confidence anyway, the speed case strengthens. |
