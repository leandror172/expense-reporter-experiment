...
- **5.R4 historical extraction — DONE.** 2022–2025 old workbooks → 1808 deduped records; training corpus 694→1788; per-year expense logs. Memory `project_workbook_extraction_5r4`.
- **PIVOT (session 36): retire workbook insertion, keep only generation** — JSONL logs become the single source of truth, `generate-workbook` the only writer. Plan: `.claude/plans/retire-insertion-keep-generation.md`. WS-0 diff validated the premise; WS-0b extracted historical income.
- **WS-A / T-11 — DONE (session 37):** multi-year log support (`parseDate` year, `scanEntries` year filter, merge script, byte-identical gate). Income decisions locked.
- **WS-C / income route — DONE & validated on real data (session 38, commits `50d09f7`…`0b28ee5`).** 3-level income model (`RevenueBlock{Category,Block,Label}`, 3-segment `incomePath`), dual-format loader (flat + nested `incomeCategories`), separate income scan from `--income-entries` (extractor `income_log.jsonl` schema, signed values), 3-level Receitas render + per-Block Listas rollup (symmetric with expense categories). Acceptance-first (`generate-income` fixture + frozen oracle); `generate-basic` re-frozen, kept FLAT for dual-format coverage. Real 2024 regen places 101/101 income records, Receitas populated. **WS-0b fix:** `extract_income.py` `fmt_date` dropped the month when the source day cell was blank (175/179 dateless) → default day `01`; income now skips dateless rows with a LOUD stderr count instead of hard-failing.
- **Next:** **WS-B** (commands → log-append); then WS-D (retire bare-name fallback, gate on type-less count ~0), WS-E (delete dead insert code). Open: promote merged `expenses_log-allyears.jsonl` to canonical + retire per-year split (deferred).
...
|------|-----------|-------|
| **WS-B commands → log-append (START HERE)** | retire-insertion plan "WS-B"; `cmd/expense-reporter/cmd/{add,auto,batch_auto,apply}.go`; `internal/apply` writer | Commands stop touching the workbook; append to `expenses_log.jsonl` via the apply writer. Entry contract carries the full typed path (so WS-D can later drop the bare-name fallback). |
| Regenerate full workbook from real logs (incl. income) | `generate-workbook --taxonomy config/taxonomy.json --entries expenses_log-allyears.jsonl --income-entries ~/workspaces/expenses/old/extracted/income_log.jsonl --year N` | WS-C proven on real 2024. Income spans 2023–2024 only (2022 absent). Watch stderr for the income dateless-skip count (~0 expected) and the type-less expense fallback count. |
| Retire bare-name fallback (T-09 / WS-D) | `[ref:taxonomy-identity-key]`; `internal/taxonomy/loader.go` `scanEntries`; stderr fallback count; taxonomy `.memories/KNOWLEDGE.md` | Gated on the type-less surface reaching ~0. Income now has its OWN `incomePath` route (WS-C done), removing one blocker. `add`/`correct` still emit type-less (to classifications.jsonl only). |
...
| Delete dead insert code (WS-E) | retire-insertion plan "WS-E"; `internal/workflow`, `internal/excel` write side, `internal/batch` insert path | Only after WS-B/WS-D. Gated on a clean WS-0 diff re-run. Verify `internal/inspect` reader needs before deleting `internal/excel/reader.go`. |
| Year-rollover workflow (T-03) | spec §1.1 + `internal/generate/.memories/QUICK.md` | Generate year N+1 skeleton from taxonomy alone; largely subsumed by the retire-insertion pivot. |
| 5.R1 (TF-IDF layer) | `project_r1_evaluation_procedure.md` memory; `data/classification/research_insights.md` | Instrumentation prerequisite still open. Corpus now 1788. |
<!-- /ref:session-reading-guide -->
