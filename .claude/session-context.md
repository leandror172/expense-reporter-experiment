...
- **WS-B (commands → log-append) — COMPLETE (slices 1–4):** `add` (s1) + `auto` (s2) + `batch-auto` (s3) + **`apply` (s4, session 44, PR #38 open)** all append via `internal/appender.ExpandAndAppend`; no classifier-fed command writes the workbook (plain `batch` still does). **Slice 4:** `appendNewRows` replaces the excel-insert half (log-first/feedback-second/downgrade-on-append-failure + nil-guard); `dryRun` gates the found+corrected feedback write; **non-destructive both-path pre-flight** (classifications before `processEntries`; expense-log only when `newRows>0`, NO `O_CREATE`); count=1 installments (T-21); deleted `--workbook`/`--backup`/`uninsertable` + dead predicates. Plan: `.claude/plans/ws-b-slice4-apply-log-append.md`.
- **Next:** review + merge **PR #38** (slice-4, 8 commits, full suite green). Then **WS-D** (retire bare-name fallback, T-09; resolve T-19 first) → **WS-E** (delete dead insert code, NARROW). PR #36 follow-ups open: **T-14** (model accuracy+speed benchmark), **T-19** (enum "none-of-these" escape hatch). Open: **T-20** expense-log dedup (apply's cross-file idempotency is best-effort), **T-21** reviewed-installment under-recording; promote merged `expenses_log-allyears.jsonl` to canonical + retire per-year split (deferred).
...
|------|-----------|-------|
| **Review + merge PR #38 (slice-4 branch, do FIRST)** | PR #38 (`feat/ws-b-slice4-apply-log-append` → master); plan `.claude/plans/ws-b-slice4-apply-log-append.md` | 8 commits, full `-tags=acceptance` suite green (1020 s). Review + merge. Point at master, don't fuss over base ([[feedback_pr_base_no_rebase_fuss]]). |
| **WS-D — retire bare-name fallback (T-09, START HERE after PR #38)** | `[ref:taxonomy-identity-key]`; `internal/taxonomy/loader.go` `scanEntries`; stderr fallback count; **tasks.md T-19** | **Unblocked by T-13; WS-B now complete.** Gate on real-data type-less surface ~0. **Coupled to T-19:** retiring the fallback leans harder on the model path, which (post-T-13) can no longer decline novel inputs — resolve the escape-hatch gap FIRST. The now-dead `BuildTypeIndex`/`LookupType` may be removed in WS-E. |
