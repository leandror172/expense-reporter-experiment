...
- **T2 HARNESS EXTRACTION — COMPLETE (Session A s58 + Session B s59).** Session A published `github.com/leandror172/acceptance-harness` v0.1.1 (public, MIT, go 1.25) + repointed career-search (TC-EXT closed, PR #7). **Session B (s59, expenses PR #48 open):** `test/harness/` deleted; expenses now depends on the module for Context/Scenario/Run + generic `verify.*`. **D1** — module owns `verify`, local domain verifiers renamed `test/verify/`→`test/expect/` (8 module-twin duplicates deleted, verified byte-identical first); scenarios import both `verify.*` (generic) + `expect.*` (domain). **D2** — `Context.{DataDir,WorkbookPath}`→`ctx.Env` accessors in `test/domain/` (`WorkbookDir` dropped, 0 refs), forwarded to the subprocess. `RequireOllama`→`test/extern/`; dead `CompareCSVExact/Fuzzy` deleted. **GATE MET:** full acceptance roster byte-identical pre/post (44 pass / 5 fail / 0 skip, same tests, same failure reasons) — the 5 reds pre-existing (T-12 dark tests + T-32 `Uber Centro` fixture-arg miss = T-34), filed. Module docs fixed (verify-collision + `TestMain` leak) + `docs/consumer/expense-reporter.md` added → **module PR #1**. **No v1.0.0 until PR #48 merges.**
- **Next: merge PR #48 + module PR #1, then tag `acceptance-harness` v1.0.0.** Product: **WS-D (T-09)** — HELD, gate-to-review not silent insert; or **T-20** dedup (independent). Also: 2 red-test fixes (**T-34** `Uber Centro` arg swap; **T-12** dark workbook-gated tests); merge career-search PR #7; `extern/llm` promotion gated on LTG; rename `Apoia-se` in workbook Referência; promote all-years log; T-24/T-25/T-27/T-28.
...
|------|-----------|-------|
| **T2 follow-ups (merge PRs + tag v1.0.0)** | expenses **PR #48**; module **PR #1** (branch `docs/session-b-followups`); `acceptance-harness/docs/consumer/expense-reporter.md`; migration plan `.claude/plans/harness-extraction-session-b.md` | Session B DONE (s59). Merge both PRs, then tag module **v1.0.0** (gated on #48 — expenses was the last boundary test). Consumer-report findings: module's `CopyTreeToWorkDir`/`DiscoverFixtures`/nested-JSON/`verify/file.go` unused by expenses; `FileUnchanged` (byte+same-name) can't express a non-deterministic "nothing written" check; `extern/llm` spec ready but gated on LTG. |
| **T-34 / T-12 — the 2+3 red acceptance tests (deterministic, no Ollama needed to diagnose)** | `.claude/scratch/phase0-roster.txt` (full diagnosis); `test/auto_log_append_test.go`; `test/auto_test.go`, `test/feedback_test.go` | **T-34:** auto_log_append_test.go passes `Uber Centro` as a literal `When:` arg (spec 0.8 + ambiguous → agreement gate rejects → no append). Swap both to `Posto Ipiranga` + fix stale comments. **T-12:** 3 dark workbook-gated tests (`RequireWorkbook` skipped them until the workbook file appeared) fail `taxonomy path not configured` — their Givens never `SetupBinaryConfig` a taxonomy. [[feedback_rename_json_tag_acceptance]] |
| **5.R2 follow-ups (CL1 trigger widening / CL2 pre-warm / alternate embedder)** | `.claude/scratch/replay-649/FINDINGS-5r2.md`; `.claude/plans/5r2-embedding-retrieval.md` § Check-later | ADOPTED s54 (miss full-path 18.8→52.5 no-think). CL1: widen the fallback to weak/ambiguous keyword matches (top score < 0.7). CL2: pre-warm (`warm-embeddings` cmd or post-`apply` hook) if the burst annoys. qwen3-embedding:8b (+2.5pp hit@5) unbenchmarked in-model. |
...
| **T-24 — --think default decision** | `.claude/t14-benchmark-report.md` "T-30"; `.claude/scratch/replay-649/FINDINGS-{model,5r2}.md` | Gate band: think-on −2.3pp. On the miss path with 5.R2 ON, think adds only +2.5pp churn at 5.5× latency. Both strata favor no-think. |
