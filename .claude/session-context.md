...
- **T-23 ROUTE REFRAMED (session 51):** gate = **external validators** (E1 recurrence-strength / E2 value-range / E3 retrieval-agreement) over **model-introspection**. Recurrence dwarfs any token signal. PR #43 (docs). NOT decided — deferred.
- **T-23 GATE + RETRIEVAL MEASURED ON REAL DATA (session 52) — the 649-replay.** Reports `.claude/scratch/replay-649/{FINDINGS,FINDINGS-model}.md`; harness `internal/classifier/replay_{retrieval,model}_test.go` (`//go:build replay`). Advisor-stress-tested; SHIP vs HOLD split:
  ...
  - **Retrieval:** miss rate 24.7%, 100% `no_keyword_match`, 160/160 misses have an in-pool neighbor → **5.R1 TF-IDF RULED OUT, 5.R2 embeddings is the lever** (softened; miss pool is the accuracy sinkhole, 18.8% full-path).
- **Next (settled): 5.R2 embeddings**, gated on the cheap **NN-retrieval precondition (T-31)** — embed the 160 misses + pool-mates, measure neighbor subcat hit-rate; weak → novel=permanent-review. Then wire the specificity/agreement gate into `IsAutoInsertable` (T-32, the WS-D unblock). **T-20** dedup independent. Open: merge PR #43; rename `Apoia-se` in workbook Referência; promote all-years log; T-24/T-25/T-27/T-28.
...
|------|-----------|-------|
| **5.R2 embeddings — NN-retrieval precondition (T-31, the settled NEXT step)** | `.claude/scratch/replay-649/FINDINGS.md` + `FINDINGS-model.md`; `data/classification/embedding-retrieval.md`; `internal/classifier/examples.go` `SelectExamples` | The 649-replay ruled OUT 5.R1 (misses are semantic zero-overlap) and points at 5.R2 — but SOFTENED: "160/160 misses have a same-subcat pool-mate" ≠ those pool-mates are embedding-retrievable (esp. grab-bag subcats). **Precondition BEFORE building:** embed the 160 no-keyword-match miss items + pool-mates (Ollama `/api/embeddings`), measure nearest-neighbor subcat hit-rate. Weak NN → novel items are permanent-review, not 5.R2. Reuse the replay harness's miss list (`retrieval.jsonl`, `n_examples==0`). |
...
| **T-23 — gate route (measured; decision still deferred)** | `.claude/plans/t23-calibration-benchmark.md` "Route decision"; `.claude/t23-strategic-implications.md`; `.claude/scratch/replay-649/FINDINGS-model.md` | Route = external validators over model-introspection (session 51). **Measured session 52:** E1 specificity works as a monotone discriminator; agreement is the strongest gate; confidence is dead; 5.R2 is the accuracy lever. Still not DECIDED — the gate wiring (T-32) + 5.R2 precondition (T-31) feed the call. |
| **T-20 — expense-log dedup (good independent pick-up)** | `internal/feedback/expense_log.go` `AppendExpense`; appender | Plain `O_APPEND`, no hash-ID dedup → re-run after partial failure double-appends. Add dedup-on-ID or `--resume`. Deterministic, no Ollama — do it while T-23 is decided. |
