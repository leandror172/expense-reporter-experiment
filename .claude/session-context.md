...
- **Next: merge PR #45; T-32** (wire the specificity/agreement gate into `IsAutoInsertable`, the WS-D unblock) — independent, parallel-able. **Harness extraction Session A** ready to execute. **T-20** dedup independent. Open: rename `Apoia-se` in workbook Referência; promote all-years log; T-24/T-25/T-27/T-28.
...
|------|-----------|-------|
| **Harness extraction Session A (T2 — create `acceptance-harness` repo + repoint career-search)** | `.claude/plans/harness-extraction-plan.md` (§3 design rules, §4 steps + caveats); companion `.claude/plans/harness-extraction-acceptance-test-comparison.md`; `career-search/dashboard/test/EXTRACTION-NOTES.md` + `dashboard/test/harness/` (the seed) | Seed from career-search's copy, NOT expenses. Strip `//go:build acceptance`; env vars over package-level flags; verify no personal data in examples. Gate: career-search's 26 deterministic tests green; closes its TC-EXT. Session B (expenses migration) is separate — plan §5. |
| **T-32 — wire the specificity/agreement gate into `IsAutoInsertable` (WS-D unblock; the settled NEXT step)** | `.claude/scratch/replay-649/FINDINGS-model.md`; `internal/classifier/decision.go` (`IsAutoInsertable`), `examples.go` (`SelectExamples` — the DISCARDED specificity), `classifier.go` (`Classify` return shape) | Session-52 replay: confidence dead; **specificity `top_score` + model⊕keyword AGREEMENT is the gate** (spec=1.0 ∧ agree → 95% subcat). Gate runs no-think. Wiring = expose match-strength (+ optionally keyword-top1) from `SelectExamples` → thread out of `Classify` → widen `IsAutoInsertable` → 3 call sites (`auto.go`×2, `batch_auto.go`). No new inference. **HOLD:** silent auto-insert is not proven (sample = confidence-selected subset) — design for gate-to-review, not silent insert. Note: 5.R2 raised miss-stratum accuracy to ~55% — still far below auto-insert precision, so the gate framing is unchanged. |
