...
- **Next: merge PR #56 → PR #57 → T-41 slice 2 (`auto` + cmd-level `parseOptions`, now genuinely the 3rd caller) → slice 3 (`batch-auto` parse-once-per-row + T-40 boundary test; decide the warning's per-run vs per-row shape, T-49) → slice 4 (`apply`, retires `canonicalDate`) → T-21 threading → T-42 monthly close.** At session start: verify verdict capture for `9de3ef267870`/`1f8d911ee6d0`/`af4aa8d39856` in `calls.jsonl`. WS-D (HELD: gate-to-review, not silent insert) → WS-E after (includes plain `batch` + `utils.ParseDate` retirement). Standing queue: T-25/T-27/T-28/T-29, T-48 (past-side year bound, deferred), Apoia-se rename, promote all-years log, `workbook_path` still names a 2025 file.
...
| **T-41 parse boundary — NEXT: slice 2 (`auto`)** | `.claude/plans/parse-boundary.md` §5 (per-slice mechanics) + §10 (decision record); `expense-reporter/internal/parse/.memories/QUICK.md` | Slices 1 merged (PRs #54/#55). Slice 2 = repoint auto.go:51/:58 to `parse.Fields` + extract cmd-level `parseOptions(yearFlag, cfg)` (third caller; cmd-level, `parse` must NOT import `config`). **s65 additions:** hoist `config.Load()` above the parse (it sits below at auto.go:69, and `ConfigYear` feeds the ladder) — this reorders error precedence; collapse `appendExpense`'s five scalars into `pe parse.ParsedExpense` (that signature is the shape that produced T-35); watch the arg transposition (`auto` is item/value/date, `Fields` is item/date/value) and assert the resolved DATE, not just success. `classify` stays OUT (read-only, never hashes). Net: `expect.JoinIDMatchesAcrossLogs` + short-`DD/MM` fixtures. Slice 3 takes T-40 as a boundary unit test AND must decide the stale-year warning's per-run vs per-row shape (T-49); slice 4 retires `canonicalDate` and precedes T-21. Deterministic suite ~8s; `-full` ~111s. |
