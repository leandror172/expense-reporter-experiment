...
- **T2 HARNESS EXTRACTION — CLOSED (Sessions A s58, B s59, closeout s60).** `github.com/leandror172/acceptance-harness` published (public, MIT); career-search repointed (PR #7 merged); expenses migrated (PR #48 merged) — `test/harness/` gone, domain split into `test/{expect,domain,extern}/`, scenarios import module `verify.*` + local `expect.*`. Module PR #1 (consumer report + doc fixes) merged. **v1.0.0 TAGGED (s60)** at `11beb2a`; expenses bumped v0.1.1→v1.0.0 (master). Pre-tag check: the "unused by expenses" surface (`CopyTreeToWorkDir`/`DiscoverFixtures`/nested-JSON/`verify/file.go`) is all USED by career-search; the `FileUnchanged` expressiveness gap is additive (future minor). Engine changes = PR upstream + version bump here.
- **SESSION 60: suite fully green + T-20 SHIPPED (PR #49 open).** (a) The 5 pre-existing red acceptance tests repaired (T-34 arg swap; T-12 de-workbooked + taxonomy Givens) → **49/49 green (1311s)**, committed to master. (b) **T-20 `batch-auto --resume` + always-on duplicate-append warning** on branch `feat/t20-batch-auto-resume`: id-count ledger (`feedback.LoadExpenseIDCounts`), `appender.PredictEntryIDs` (shared `expandEntries` — drift guard) skips fully-logged rows BEFORE the ~12s LLM call; partially-logged series → review (never auto-completed); log stays append-only by default (legit duplicate triples exist). Advisor = **Opus xhigh subagent** (Fable has no `advisor()`; memory updated) — review `.claude/advisor-t20-resume-dedup.md`; implementation by new `impl-opus-xhigh` agent; report `.claude/t20-resume-implementation-report.md` (§ "batch-auto --resume semantics", becomes `ref:batch-auto-resume` once #49 merges). Post-change full suite green (779s, 5 new acceptance tests).
- **Next: merge PR #49 (T-20).** Product: **WS-D (T-09)** — HELD, gate-to-review not silent insert; then WS-E. Backlog candidates pending amend: T-35 (auto raw-vs-normalized date → divergent join id across logs), T-36 (cosmetic sweep). Also: T-24/T-25/T-27/T-28; rename `Apoia-se` in workbook Referência; promote all-years log.
...
|------|-----------|-------|
| **Merge PR #49 (T-20 --resume) — first thing next session** | PR #49; `.claude/t20-resume-implementation-report.md`; `.claude/advisor-t20-resume-dedup.md` | Branch `feat/t20-batch-auto-resume`. Full suite green pre-PR (779s, 54 tests). After merge, the report's semantics block becomes `ref:batch-auto-resume`. Docs/memories for T-20 ride the PR. |
