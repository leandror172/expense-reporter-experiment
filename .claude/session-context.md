...
- **T-30 leaf-first A/B — DONE (session 50) → D4 = HOLD (do not adopt for accuracy).** Benchmark-only harness `.claude/scratch/leaf-first-ab/`; 4 cells (few-shot OFF/ON × no-think/think-on, n=300 paired, McNemar). GBNF leaf-then-type generation order confirmed. Full-path Δ decays **+12.7 → +8.7 → +3.3 → +1.7 (production = few-shot ON + think-on, p=0.46 NOT sig)** — few-shot retrieval already fixes the same wrong-leaf errors, so stacking is sub-additive. Type Δ survives longest (sig 3/4 cells) but washes out in prod (+2.3, p=0.30). Not worse; just not worth productionizing. **Still the T-23 precondition ONLY for the logprob/perplexity signal (OPEN/possibly-unneeded for ensemble).** `type_crosscheck` not a usable signal. Couples to T-27. Report `.claude/t14-benchmark-report.md` "T-30".
- **Next:** **T-23** calibration benchmark — decide the signal route first: logprob/perplexity (2a/2b, needs a leaf-first base) vs **ensemble-agreement** (3, needs NO leaf-first) vs recurrence-first fallback; risk-coverage on the T-14 sample, stratify leaky/clean. Then **WS-D (T-09)** retire bare-name fallback → **WS-E** delete dead insert code. Independent: **T-20** dedup. Open: merge PR #42; rename `Apoia-se` in workbook Referência (durability); re-justify or drop **T-27** (few-shot already recovers most wrong-leaf errors); T-28/T-21/T-24/T-25; promote all-years log (deferred).
...
### Leaf-First & Token-Confidence (T-23 exploration → T-30 A/B, sessions 49–50)
...
|------|-----------|-------|
| **T-23 — calibration/gate rethink (blocks WS-D; the live front)** | `.claude/plans/t23-calibration-benchmark.md`; `.claude/t23-logprob-confidence-probe.md`; `.claude/t14-benchmark-report.md` ("T-30" + calibration + "Results — think-on"); `internal/classifier/decision.go`; `config.json` `auto_insert_excluded` | Confidence uninformative (86–95% of errors ≥0.85); descriptions (T-22/T-26) don't fix it; **leaf-first (T-30) is HELD — no accuracy case.** DECIDE THE SIGNAL ROUTE FIRST: logprob/perplexity (2a/2b, needs a leaf-first base — harness at `.claude/scratch/leaf-first-ab/`) vs **ensemble-agreement** (3, needs NO leaf-first) vs recurrence-first fallback. Risk-coverage primary, stratify leaky/clean. Ollama reports PRE-mask logprobs. 2b engine-PoC subagent deferred (ask model before spawning). The route choice decides whether leaf-first is ever revived. |
| **T-30 leaf-first — DONE (session 50), HOLD** | `.claude/plans/leaf-first-classification.md` "Execution log"; `.claude/t14-benchmark-report.md` "T-30"; harness `.claude/scratch/leaf-first-ab/` (`run_leaf_ab.py`/`analyze_leaf_ab.py`/`.memories/QUICK.md`) | Reference only — NOT pending. D4 = HOLD (no significant production gain; few-shot already fixes wrong-leaf errors). Re-usable harness if T-23 needs a leaf-first base. Enum gain decays +12.7→+1.7 (prod, ns) across the 4 few-shot×think cells. |
...
| **T-20 — expense-log dedup (good independent pick-up)** | `internal/feedback/expense_log.go` `AppendExpense`; appender | Plain `O_APPEND`, no hash-ID dedup → re-run after partial failure double-appends. Add dedup-on-ID or `--resume` before volume use. Deterministic, no Ollama — do it while T-23 signal-route is being decided. |
