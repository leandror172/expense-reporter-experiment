...
- **WS-B (commands → log-append) — IN PROGRESS:** `add` (slice 1, `d6d66c8`) + `auto` (slice 2, `597c649`) DONE — but using the OLD feature-dict-category / taxonomy-type split resolution (emits type-less lines on divergent categories → fixed by T-13). New `internal/appender.ExpandAndAppend` = single append-time writer; installments expand at append time (N dated lines, cross-year carries next-year date, `rollover.csv` retired); no workbook writes. **Remaining: `batch-auto`, `apply` (write-half deletion).**
- **T-13 — DESIGNED & de-risked (session 40), not yet implemented.** Reframed to "classifier predicts the full `(type,category,subcategory)` path against taxonomy.json" via an atomic 112-path enum (smoke-tested 100% valid, ~6s on `my-classifier-qcoder`); also resolves the 5 multi-type leaves the `(cat,sub)` lookup can't. **WS-B resolution-correctness sub-slice** — retrofits slices 1–2 + precondition for WS-D. All 4 decisions locked; `IRFF→IRRF` taxonomy fix applied (gitignored/local-only — must persist). Plan: `.claude/plans/t13-classifier-full-path.md`.
- **Next:** implement T-13 (`internal/taxonomy` `PathEnum`/`SplitPath`; classifier emits path + enum schema + `Type` on Result; retrofit `add`/`auto`; few-shot type from training `source`), then WS-B slice 3 `batch-auto` (on predicted-path) + slice 4 `apply`; then WS-D (retire bare-name fallback, gate on type-less count ~0), WS-E (delete dead insert code). Open: promote merged `expenses_log-allyears.jsonl` to canonical + retire per-year split (deferred).
...
|------|-----------|-------|
| **T-13 — classifier predicts full path (START HERE; WS-B resolution-correctness + WS-D prereq)** | `.claude/plans/t13-classifier-full-path.md` (all 4 decisions + §4 work breakdown); `internal/classifier/classifier.go` (schema `:128`, prompt `:235`, `Result` `:15`); `internal/taxonomy/lookup.go`; `cmd/.../add.go` `:68`/`:176`, `auto.go` `:198` | Add `PathEnum`/`SplitPath` to `internal/taxonomy` (Option 2B, no cycle); classifier renders 3-level tree + `path` enum + `Type` on Result; few-shot type from training `source` sheet-name (~96%); `add` = taxonomy walk + `--type` hybrid. `IRFF→IRRF` taxonomy fix already applied (gitignored/local-only). Smoke-test: `scratchpad/enum_smoketest.py`. |
| WS-B slice 3 `batch-auto` → log-append | retire-insertion plan "WS-B progress"; `cmd/.../batch_auto.go`; `internal/appender/appender.go`; slice-1/2 commits `d6d66c8`/`597c649` | Build on predicted-path (T-13) from the start. Drop the workbook-insert branch + `rollover.csv`; keep classified/review CSV → review → apply. Reuse `ExpandAndAppend`. Run target tests in ISOLATION with `-v` (full suite hits the T-08 600s flake). |
...
| Slice-2 loose ends (T-12) + stale memory | plan "WS-B progress" (LOOSE ENDS + STALE .memory blocks) | Rewire `test/auto_test.go` workbook-gated cases (LOW path uncovered); rename `auto.go:162` `✓ Inserted`. Refresh `internal/feedback/.memories`, add `internal/appender/.memories`, update repo `.memories/QUICK.md`. |
| Retire bare-name fallback (T-09 / WS-D) | `[ref:taxonomy-identity-key]`; `internal/taxonomy/loader.go` `scanEntries`; stderr fallback count | Gated on type-less surface ~0. Income has its own `incomePath` route (WS-C done). Blocked by T-13. |
| Delete dead insert code (WS-E) | plan "WS-E"; `internal/workflow`, `internal/excel` write side, `internal/batch` insert path | Only after WS-B/WS-D + a clean WS-0 diff re-run. Verify `internal/inspect` reader needs before deleting `internal/excel/reader.go`. |
