...
- **T-22 — DONE (session 47), PR #42 MERGED.** Type-level taxonomy descriptions merged into the classifier prompt from a tracked sidecar `config/type-descriptions.json`. No-think A/B: **English +6.0 pp TYPE accuracy** (74.3→80.3), +2.6 pp full-path; Portuguese dropped; **calibration UNCHANGED — not a gate fix**. Leaf-level extension = T-27.
...
- **T-23 EXPLORED (session 49): logprob-confidence probe + leaf-first spin-off.** Ollama 0.17.5 exposes logprobs but reports them PRE-grammar-mask (raw, not renormalized) → can't read P-over-legal-paths directly; type-first enum makes the margin unreadable, LEAF-FIRST makes it legible. Report `.claude/t23-logprob-confidence-probe.md`; plans `.claude/plans/{leaf-first-classification,t23-calibration-benchmark}.md`.
- **T-30 leaf-first A/B — DONE (session 50) → D4 = HOLD (do not adopt for accuracy).** Benchmark-only harness `.claude/scratch/leaf-first-ab/`; 4 cells (few-shot OFF/ON × no-think/think-on, n=300 paired, McNemar). Full-path Δ decays **+12.7 → +8.7 → +3.3 → +1.7 (production, p=0.46 NOT sig)** — few-shot retrieval already fixes the same wrong-leaf errors (sub-additive). `type_crosscheck` not a usable signal. Report `.claude/t14-benchmark-report.md` "T-30".
- **T-23 ROUTE REFRAMED (session 51).** Validated the HOLD (recomputed prod cell; novel stratum +2.6 n.s.) + read the classifier end-to-end → gate is confidence-only (`IsAutoInsertable`) and the recurrence signal is computed-then-DISCARDED in `SelectExamples`. Advisor-reviewed reframe: gate = **EXTERNAL VALIDATORS** (E1 recurrence-strength / E2 value-range / E3 retrieval-agreement) over **MODEL-INTROSPECTION** (M1 ensemble / M2 logprob). Recurrence (~70% vs ~49%, ~25pp) dwarfs any token signal + is orthogonal to self-confidence. Route: `.claude/plans/t23-calibration-benchmark.md` "Route decision"; strategy: `.claude/t23-strategic-implications.md`. **PR #43 (docs). NOT decided — deferred.**
- **REAL-DATA FINDING (session 51):** `classifications.jsonl` = **649 human-verified `(item→actual)` labels** from real 2025 usage (review/apply, WS-B s44). Real workflow has been **review-first** → the auto-insert gate was never battle-tested on real data (corroborates the reframe). **GOTCHA:** all rows `model="review"` + DEFAULTED confidence (0.95×502) + selection-biased → do NOT measure the gate off the stored fields (naive read = artifactual 7.2% flat curve).
- **Next:** **REPLAY the 649 labeled expenses through the current classifier** — the unblocked measurement that decides BOTH the T-23 gate (real risk–coverage of recurrence-strength/confidence → WS-D readiness) AND the 5.R1 TF-IDF trigger (keyword miss rate), in ONE pass. Miss-rate part is cheap (replay `SelectExamples`, no Ollama); confidence part ~16 min no-think. **Exclude example-pool self-matches** (leakage; split leaky-vs-clean). Then recurrence-first gate → **WS-D (T-09)** → **WS-E**. Independent now: **T-20** dedup. Strategic (`t23-strategic-implications.md` §6): protect the review→feedback loop; retrieval (5.R2 > 5.R1) is the accuracy lever; deprioritize prompt tuning (T-27). Open: merge PR #43; rename `Apoia-se` in workbook Referência; T-28/T-21/T-24/T-25; promote all-years log.
...
|------|-----------|-------|
| **T-23 — gate rethink (blocks WS-D; the live front)** | `.claude/plans/t23-calibration-benchmark.md` "Route decision"; `.claude/t23-strategic-implications.md`; `internal/classifier/decision.go` (as-is gate) + `examples.go` (`SelectExamples` — the DISCARDED recurrence signal); `.claude/t14-benchmark-report.md` "T-30" | **REFRAMED (session 51):** gate = **external validators** (E1 recurrence-strength / E2 value-range / E3 retrieval-agreement) over **model-introspection** (ensemble / logprob). Recurrence (~70% vs ~49%) dwarfs any token signal + is orthogonal to self-confidence. **FIRST STUDY (unblocked): E1 recurrence-strength risk–coverage stratified recurrent/novel** (+ E2 value-range negative flag); the curve decides whether an introspection signal (ensemble first, on the NOVEL pool) is needed. As-is gate = confidence≥0.85 + exclusion only; wiring E1 = expose from `SelectExamples` → thread out of `Classify` → widen `IsAutoInsertable` → 3 call sites (no new inference). Leaf-first parked (revive only if novel-pool study picks logprob). Do NOT build on E3 pre-stability-test; value-range = NEGATIVE flag only. |
| **Real-data replay measurement (the unblocked next step — feeds T-23 AND 5.R1)** | `expense-reporter/classifications.jsonl` (649 real labels); `internal/classifier/examples.go` `SelectExamples`; `.claude/scratch/{t14-benchmark,leaf-first-ab}/` (harness to reuse); `.claude/t23-strategic-implications.md` §6 | `classifications.jsonl` = 649 human-verified `(item→actual)` labels from real 2025 usage — but `model="review"` + DEFAULTED confidence (0.95×502) + selection-biased → **do NOT read the gate off the stored fields** (artifact). **REPLAY the 649 through the current classifier** → real risk–coverage (gate/WS-D) + keyword miss rate (5.R1 trigger) in one pass. Miss-rate part = replay `SelectExamples` only (no Ollama); confidence part ~16 min no-think. **Exclude example-pool self-matches** (leakage; split leaky-vs-clean). |
| **T-30 leaf-first — DONE (session 50), HOLD** | `.claude/plans/leaf-first-classification.md` "Execution log"; `.claude/t14-benchmark-report.md` "T-30"; harness `.claude/scratch/leaf-first-ab/` (`run_leaf_ab.py`/`analyze_leaf_ab.py`/`.memories/QUICK.md`) | Reference only — NOT pending. D4 = HOLD (no significant production gain; few-shot already fixes wrong-leaf errors). Re-usable harness if T-23 revives a leaf-first base. Enum gain decays +12.7→+1.7 (prod, ns) across the 4 few-shot×think cells. |
| **T-27 — leaf-level descriptions (RE-JUSTIFY or drop)** | T-22 addendum; T-30 report section; `writeTaxonomyTree`/`descriptions.go`; `config/type-descriptions.json` | **Motivation undercut by T-30:** few-shot retrieval already recovers most "right sheet, wrong leaf" errors in production, the same class per-leaf descriptions target. Before building, re-justify on few-shot-ON evidence (does anything still move the wrong-leaf rate with few-shot on?) — or drop. Needs prompt-size/latency measurement; extends the sidecar schema beyond the flat type→desc map. |
| **T-28 — alternative classification strategies** | `internal/classifier/classifier.go` (prompt + `format`/GBNF); T-13 report; t14 "T-30" | Explore beyond one-shot atomic-enum: (a) two-stage — leaf-first (T-30) IS one variant, now measured (HOLD for accuracy); (b) two-pass grammar — classify UNCONSTRAINED first, then re-ask WITH the grammar only if the free-form answer doesn't match the tree (couples to T-23's "unconstrained-first" confidence route). Measure vs baseline on the T-14 sample. |
| **T-24 — --think default decision** | `.claude/t14-benchmark-report.md` "Results — think-on" + "T-30"; T-22 addendum; `sigtest_t26.py` | Data now exists. Descriptions on → think-on's sentinel advantage gone → accuracy-vs-speed call: ~+2.7 pp full-path for ~10× latency (likely not sig on n=300). If T-23 moves safety off confidence (or ensemble needs no-think for cost), the no-think case strengthens. |
| **T-20 — expense-log dedup (good independent pick-up)** | `internal/feedback/expense_log.go` `AppendExpense`; appender | Plain `O_APPEND`, no hash-ID dedup → re-run after partial failure double-appends. Add dedup-on-ID or `--resume` before volume use. Deterministic, no Ollama — do it while T-23 is being decided. |
| **WS-D — retire bare-name fallback (T-09, after T-23)** | `[ref:taxonomy-identity-key]`; `internal/taxonomy/loader.go` `scanEntries`; stderr fallback count | **Gate NOT passed in T-14/T-26** — ~37% wrong full paths behind a gate that stops ~9% of them. Requires T-23 (recurrence-first gate → novel rows never reach the append path) + real-data type-less surface ~0. |
...
| Retrieval upgrade — embeddings 5.R2 > TF-IDF 5.R1 (the real accuracy lever) | `data/classification/{embedding-retrieval,tfidf-retrieval}.md`; `t23-strategic-implications.md` §2c | Strategic: prompt/enum tuning has hit diminishing returns; recurrence/retrieval is the lever. Embeddings turn novel items into recurring-like (semantic near-neighbors), directly shrinking the ~49% novel pool. Higher ROI than more prompt work. |
