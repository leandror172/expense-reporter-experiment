...
- **T-26 — DONE (session 48).** Think-on confirmation of T-22 (`run_t26.sh` sidecar-toggle; `sigtest_t26.py` paired McNemar/Fisher). Type lift is **direction-consistent across runs** (+6.0 no-think, +2.3 think-on) → adoption holds; but every single-run delta is within noise (type-all p=0.36). **Retracted 2 report findings as noise** (novel-concentration — leaky/novel sign-flips across runs; sentinel-suppression — n=20, p=0.11). **Calibration still broken (86–88% HC-wrong) → gate untouched; T-23 stands.** Report: "Results — think-on".
- **Next:** **T-23** calibration/gate rethink (the WS-D blocker — T-22/T-26 confirmed descriptions don't fix the gate) → **WS-D (T-09)** retire bare-name fallback → **WS-E** delete dead insert code. Open: merge PR #42 (updated w/ T-26); T-27 (leaf-level descriptions) · T-28 (alt classification strategies) · T-20 dedup · T-21 installment under-recording · T-24 --think default · T-25 q35 grammar-bug upstream + persona cleanup; promote merged all-years log (deferred).
...
|------|-----------|-------|
| **Merge PR #42 (T-22)** | PR #42; branch `feat/t22-type-descriptions` | Type-descriptions via tracked sidecar; PR body updated with the T-26 think-on results. Point at master, don't fuss over base ([[feedback_pr_base_no_rebase_fuss]]). |
| **T-23 — calibration/gate rethink (blocks WS-D, do FIRST)** | `.claude/t14-benchmark-report.md` (calibration section + T-22 addendum + "Results — think-on"); `internal/classifier/decision.go`; `config.json` `auto_insert_excluded` | Confidence is uninformative (86–95% of errors ≥0.85). **T-22 AND T-26 confirmed descriptions do NOT fix this, in either think mode.** Options: threshold on margin/agreement instead of raw confidence, few-shot pairs with varied confidence (0.95 anchor suspicion), review-first default for non-recurring items (recurrence is a better safety signal than confidence), stronger exclusions. |
| **T-27 — leaf-level descriptions (extends T-22)** | T-22 addendum; `writeTaxonomyTree`/`descriptions.go`; `config/type-descriptions.json` | Type-level descriptions left "wrong leaf" errors untouched. Per-leaf descriptions (×112) target those — **motivate via T-14's robust "right sheet, wrong leaf" error class, NOT T-26's noisy novel-type +8.6 pp.** Need prompt-size/latency measurement first; would extend the sidecar schema beyond the flat type→desc map. |
...
| **T-24 — --think default decision** | `.claude/t14-benchmark-report.md` "Results — think-on"; T-22 addendum; `sigtest_t26.py` | Data now exists. With descriptions on, think-on's sentinel advantage is gone (weak in every condition) → the call is accuracy-vs-speed: ~+2.7 pp full-path for ~10× latency (likely not sig on n=300). If T-23 moves safety off confidence, the no-think speed case strengthens. |
