...
- **T-23 GATE + RETRIEVAL MEASURED ON REAL DATA (session 52) — the 649-replay.** Reports `.claude/scratch/replay-649/{FINDINGS,FINDINGS-model}.md`; harness `internal/classifier/replay_{retrieval,model}_test.go` (`//go:build replay`). Advisor-stress-tested; SHIP vs HOLD split:
  - **SHIP (bias-robust):** confidence is DEAD as a gate (52.5→56.3% flat; today's conf≥0.85 admits 97% at coin-flip 53%). **Specificity (`top_score`) is a real monotone discriminator → replace confidence** (52→87% all / 44→83% clean full-path). **Best gate = AGREEMENT: `top_score==1.0` ∧ model==keyword-top1 → 95.0% subcat** (201 rows) — clears the bar; keyword-top1 beats the 8B model on subcat at spec=1.0 (91.0 vs 87.8). **Gate runs no-think** (shape robust to think mode, spread 0.2pp; 10× faster, no loss).
  - **HOLD (absolute level unmeasurable):** the 649 is a **confidence-selected review SUBSET** — `expenses_log.jsonl` has 725 unique expenses, only 347 reviewed; **378 (52%) bypassed review, unlabeled** → true full-stream precision is unmeasurable. So "no unattended auto-insert" + WS-D silent-insert scoping are HELD, not proven.
  - **Retrieval:** miss rate 24.7%, 100% `no_keyword_match`, 160/160 misses have an in-pool neighbor → **5.R1 TF-IDF RULED OUT, 5.R2 embeddings is the lever** (miss pool is the accuracy sinkhole, 18.8% full-path).
- **T-31 NN PRECONDITION DONE (session 53) — GO for 5.R2.** 5-model embedding NN probe over the 160 misses vs the full 1,744 pool (faithful merge, LOO, text-only): hit@5 qwen3-embedding:8b 65.0% / **arctic-embed2 62.5% (best hit@1 50%, 1.2 GB — the practical pick)** / bge-m3 61.3% / embeddinggemma 57.5% / nomic 41.2% (multilingual > size). Robust to Diversos-exclusion + unique-item cuts; clears the pre-committed ≥60% bar. ~35% permanent-review residue remains. Report `.claude/scratch/replay-649/FINDINGS-nn.md`; probe `nn_precondition.py`. PR #44.
- **Next: build 5.R2** — embed-on-miss cascade layer in `internal/classifier` (arctic-embed2, K=5, disk cache), A/B final accuracy via the replay harness. **T-32** (wire the specificity/agreement gate into `IsAutoInsertable`, the WS-D unblock) is independent and parallel-able. **T-20** dedup independent. Open: merge PR #44; rename `Apoia-se` in workbook Referência; promote all-years log; T-24/T-25/T-27/T-28.
...
|------|-----------|-------|
| **5.R2 embeddings build (the settled NEXT step — T-31 precondition PASSED)** | `.claude/scratch/replay-649/FINDINGS-nn.md` (+ `FINDINGS.md`); `data/classification/embedding-retrieval.md`; `internal/classifier/examples.go` `SelectExamples` + `classifier.go:95-106` (pool wiring) | T-31 (session 53): GO — NN hit@5 62–65% on the 160 no-keyword-match misses (full 1,744 pool, LOO). Build = embed-on-miss cascade layer: Ollama `/api/embeddings`, disk-cached pool vectors, cosine top-K → few-shot injection when keywords miss. Model: **snowflake-arctic-embed2** (1.2 GB, best hit@1 50%); alternate qwen3-embedding:8b (+2.5pp hit@5, 4.7 GB) in the A/B. **K=5 mandatory** (hit@1 ≤50%). A/B final classifier accuracy on the same replay harness (`replay_model_test.go`); ~35% of misses stay unreachable → permanent-review residue. |
| **T-32 — wire the specificity/agreement gate into `IsAutoInsertable` (WS-D unblock)** | `.claude/scratch/replay-649/FINDINGS-model.md`; `internal/classifier/decision.go` (`IsAutoInsertable`), `examples.go` (`SelectExamples` — the DISCARDED specificity), `classifier.go` (`Classify` return shape) | Session-52 replay: confidence dead; **specificity `top_score` + model⊕keyword AGREEMENT is the gate** (spec=1.0 ∧ agree → 95% subcat). Gate runs no-think. Wiring = expose match-strength (+ optionally keyword-top1) from `SelectExamples` → thread out of `Classify` → widen `IsAutoInsertable` → 3 call sites (`auto.go`×2, `batch_auto.go`). No new inference. **HOLD:** silent auto-insert is not proven (sample = confidence-selected subset) — design for gate-to-review, not silent insert. |
| **T-23 — gate route (measured; decision still deferred)** | `.claude/plans/t23-calibration-benchmark.md` "Route decision"; `.claude/t23-strategic-implications.md`; `.claude/scratch/replay-649/FINDINGS-model.md` | Route = external validators over model-introspection (session 51). **Measured session 52:** E1 specificity works as a monotone discriminator; agreement is the strongest gate; confidence is dead; 5.R2 is the accuracy lever. Still not DECIDED — the gate wiring (T-32) + the 5.R2 build feed the call. |
...
| **WS-D — retire bare-name fallback (T-09, after the gate lands)** | `[ref:taxonomy-identity-key]`; `internal/taxonomy/loader.go` `scanEntries`; stderr fallback count | Gated on a protective gate (T-32) so novel rows never reach the append path. Session-52 replay confirms the recurrence-first framing (leaky 63% vs clean 44%). |
...
| **T-24 — --think default decision** | `.claude/t14-benchmark-report.md` "T-30"; `.claude/scratch/replay-649/FINDINGS-model.md` (think-on top-band) | Session-52: on the high-specificity gate band, think-on is −2.3pp (WORSE) and shape is unchanged → the gate should run no-think. Strengthens a no-think default for the recurring/high-spec path. |
