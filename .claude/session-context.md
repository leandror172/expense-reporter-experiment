...
- **Next:** **T-23 recurrence-strength study** — cheapest, unblocked first move (E1 risk–coverage stratified recurrent/novel, + E2 value-range negative flag); the curve decides whether any model-introspection signal (ensemble first, on the NOVEL pool) is still needed. Then **WS-D (T-09)** retire bare-name fallback → **WS-E**. Independent: **T-20** dedup. Strategic re-prioritization (`t23-strategic-implications.md`): protect the review→feedback loop; retrieval (5.R2 embeddings > 5.R1 TF-IDF) is the real accuracy lever; deprioritize prompt tuning (T-27 re-justify or drop). Open: merge PR #43; rename `Apoia-se` in workbook Referência; T-28/T-21/T-24/T-25; promote all-years log.
