...
- **WS-B (commands → log-append) — IN PROGRESS (session 39):** `add` (slice 1, `d6d66c8`) + `auto` (slice 2, `597c649`) DONE. New `internal/appender.ExpandAndAppend` = single append-time writer; installments expand at append time (N dated lines, cross-year carries next-year date, `rollover.csv` retired); no workbook writes from `add`/`auto`. **Remaining: `batch-auto`, `apply` (write-half deletion).**
- **Next:** finish WS-B (`batch-auto`, `apply`); then WS-D (retire bare-name fallback, gate on type-less count ~0 — blocked by T-13 category/type divergence), WS-E (delete dead insert code). Open: promote merged `expenses_log-allyears.jsonl` to canonical + retire per-year split (deferred).
...
|------|-----------|-------|
| **WS-B slice 3 `batch-auto` → log-append (START HERE)** | retire-insertion plan "WS-B progress"; `cmd/expense-reporter/cmd/batch_auto.go`; `internal/appender/appender.go` (the helper to reuse); slice-1/2 commits `d6d66c8`/`597c649` | Drop the workbook-insert branch + `rollover.csv`; keep classified/review CSV → review → apply. Reuse `ExpandAndAppend`. Run target tests in ISOLATION with `-v` (full suite hits the T-08 600s flake). |
...
| Category/type single-source (T-13, WS-D prereq) | plan deferred finding #4; `internal/taxonomy/.memories/KNOWLEDGE.md` (two-source note) | `add`/`auto` resolve category from feature dict, type from taxonomy.json → divergent categories emit type-less lines. taxonomy.json's `types[].categories[].subcategories[]` tree can become the single source. |
