...
| `cmd` | `expense-reporter/cmd/expense-reporter/cmd/` | Cobra CLI subcommands (add, batch, version; +classify/auto/batch-auto in Layer 5; +generate-workbook in Phase G); `parse_boundary.go` = command-layer helpers over `internal/parse` (`warnIfStaleConfiguredYear`; `parseOptions` lands here in T-41 slice 2) |
