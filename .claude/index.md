...
| `cmd` | `expense-reporter/cmd/expense-reporter/cmd/` | Cobra CLI subcommands (add, batch, version; +classify/auto/batch-auto in Layer 5; +generate-workbook in Phase G; **+telegram-import** in T-75 — `telegram_import.go`: `--dry-run` prints the per-bucket report with ids, `writeBucketReport`; without `--dry-run` it returns an error naming step 3 until the writers land); `parse_boundary.go` = command-layer helpers over `internal/parse`: `parseOptions` (builds the year ladder from `--year` + config; no seam — all callers want the identical ladder), `warnIfStaleConfiguredYear`, `describeParseFailure` (adds the installment-syntax hint a value rejection needs, and never replaces the boundary's reason) |
...
| `t75_oracle_probe.py` | `.claude/scratch/t75_oracle_probe.py` | **Read-only (session 75)** — regenerates every number in `.claude/t75-oracle-viability.md`: converts the Telegram export naively, multiset-matches `(date,value)` against the live log, classifies the residue by named cause, checks installment-series completeness by ITEM, and ends with a **perturbation check** that must move the residue by exactly 1 (a comparison that cannot fail carries no information). Ports `pkg/utils/currency.go` rather than shelling out — divergence then inflates the miss count instead of hiding one. Paths relative to `__file__`; override with `EXPORT_JSON` / `EXPENSE_LOG`. ⚠️ Match on the multiset, never a set and never on item — s74's 8 same-day duplicates and s72's `San michel` x3 both die in a set |
| `t75_expected_buckets.py` | `.claude/scratch/t75_expected_buckets.py` | **T-75 step-1 gate, half 1 (s75)** — writes the PROBE's per-message-id bucket as a TSV (`id  class  bucket  reason`; ids and labels only, never text) by importing `t75_oracle_probe.py`'s own parsers, so the expectation is the probe's verdict and not a second opinion. Also prints the date/value shape census (which is how the two `DD/MM/25` messages were found). `python3 t75_expected_buckets.py OUT.tsv`; `EXPORT_JSON` overrides the export path |
| `t75_gate_compare.py` | `.claude/scratch/t75_gate_compare.py` | **T-75 step-1 gate, half 2 (s75)** — diffs `telegram-import … --dry-run` output against that TSV PER ID (converted = the complement of every listed id; the total and converted lines are cross-checked), exit 1 on any difference. **`--perturb` flips one expected id and REQUIRES exactly one difference** — the s74 rule: a comparison is evidence only once it has been seen to fail. 2025 result: 393/393, perturbation moves it by 1. Run both on the 2026 export BEFORE trusting the converter on it |
