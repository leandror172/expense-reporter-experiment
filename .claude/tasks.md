...
- [ ] (T-13) **Single-source category/type resolution for `add`/`auto`** — both resolve category from the feature dict but type from `taxonomy.json`; divergent categories (`Fixas – Impostos`, `Fixas – Saúde`) silently emit type-less lines, feeding the bare-name fallback WS-D wants to retire. Make taxonomy.json's `types[].categories[].subcategories[]` tree the single source. WS-D prerequisite.
<!-- /ref:deferred -->
