...
- [ ] (T-14) **T-13 live-model verification (gates WS-D)** — the new full-path classifier code has only ever run against hand-crafted mock Ollama JSON; the 112-enum "100% valid" evidence is the session-40 `enum_smoketest.py` (a separate Python script, NOT the Go `buildResponseSchema`/`buildSystemPrompt`/`parseResponse`). When an enum-capable model (`my-classifier-qcoder`) is up and `data/classification/` is populated, run the real binary once: `auto "Estacionamento shopping" 18,00 15/04 --json` + a 2-line `batch-auto --dry-run`. Confirm (a) no Ollama 4xx/5xx on the Go-generated schema, (b) the returned `path` splits to a NON-EMPTY type, (c) an ambiguous leaf (Dentista/Estacionamento) gets a context-right type. This is the WS-D precondition — "real-data type-less count ~0" is unmeasurable until the live path runs once.
<!-- /ref:deferred -->
