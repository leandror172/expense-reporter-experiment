// Package inspect is the structural-dump core shared by cmd/workbook-inspect
// and test verifiers: it dumps the full structural map of an expense workbook
// (values, formulas, styles, merges, row classification) to JSON files.
package inspect
...
// DumpWorkbook opens the workbook, creates outputDir, and writes manifest.json
// plus one <SheetName>.json per sheet. Sheets that fail to dump are warned to
// stderr and skipped so a partially-dumpable workbook still produces output.
func DumpWorkbook(workbookPath, outputDir string) error {
	if err := os.MkdirAll(outputDir, 0o755); err != nil {
		return fmt.Errorf("cannot create output directory: %w", err)
	}
	wb, err := excelize.OpenFile(workbookPath)
	if err != nil {
		return fmt.Errorf("cannot open workbook: %w", err)
	}
	defer wb.Close()
	return dumpWorkbook(wb, workbookPath, outputDir)
}
